...
class CustomLossFunction:
    def __init__(self, gamma=2, lambda_penalty=0.02, threshold=0.34):
        self.gamma = gamma
        self.lambda_penalty = lambda_penalty
        self.threshold = threshold

    def focal_ranking_loss(self, reward_scores, labels):
        # Determine the predicted class
        predicted_class = torch.argmax(reward_scores, dim=1)
        softmax_probs = F.softmax(reward_scores, dim=1)
        
        # Extract the probability of the correct class for each element in the batch
        p = softmax_probs.gather(1, labels.view(-1, 1)).squeeze()
        
        # Check if the prediction is correct
        correct = (predicted_class == labels).float()
        
        # Apply the max(0, p - threshold) and the difficulty decay coefficient
        decay = (1 - 2 * torch.clamp(p - self.threshold, min=0)) ** self.gamma
        
        # Adjust the decay based on correctness: Increase penalty if incorrect
        decay = torch.where(correct == 1, decay, torch.ones_like(decay))
        
        # Calculate the focal ranking loss
        ranking_loss = -(decay * torch.log(p))  # Avoid log(0) by adding a small value
        
        # Logarithmic Barrier Penalty
        penalty_loss = self.lambda_penalty * (
            -torch.log(torch.clamp(5 - reward_scores, min=1e-6))
            - torch.log(torch.clamp(reward_scores + 5, min=1e-6))
        ).mean()
        
        # Total Loss: Ranking Loss + Penalty
        total_loss = ranking_loss.mean() + penalty_loss
...
class InternLM2ForSequenceClassification(InternLM2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.model = InternLM2Model(config)
        self.v_head = nn.Linear(config.hidden_size, self.num_labels, bias=False)
    ...
    def forward(
        ...
        # Get the indices of the end tokens
        ends = attention_mask.cumsum(dim=1).argmax(dim=1).view(-1, 1, 1)  # Shape: [batch_size, 1, 1]
        
        ends = ends.to(hidden_states.device)
        
        # Now gather the reward scores for all three logits
        reward_scores = torch.gather(hidden_states, 1, ends.expand(-1, -1, hidden_states.size(-1)))  # Shape: [batch_size, 1, 3]

        # Squeeze the middle dimension if needed (optional)
        reward_scores = reward_scores.squeeze(1)
        
        loss = None
        if labels is not None:
            labels = labels.to(hidden_states.device)
            loss = self.custom_loss.focal_ranking_loss(reward_scores, labels)
        
        if not return_dict:
            output = (reward_scores,) + transformer_outputs[1:]
            return ((loss,) + output) if loss is not None else output
